...
varying vec4 fDiffuse;          // diffuse reflectivity (color)
...
varying vec4 fTangent;          // surface tangent vector t1
varying vec2 fTexCoord;         // texture coordinates
...
void main()
{
  ...
  vec4 tangent = vec4(vTangent.xyz, 0.0);
  ...
  fNormal = normalize(vModelViewMatrix * normal);  // assume uniform scale
  fTangent = normalize(vModelViewMatrix * tangent);
  ...
  fDiffuse = vec4(vColor, 1.0);
  fSpecular = vec4(vSpecular, 1.0);
