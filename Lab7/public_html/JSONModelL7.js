...
function JSONModel(gl, modelURL) {
  ...
  this.scaleSliderID = "gl-canvas-scale-slider-output";
  ...
  if (this.hasTexCoords) {
    ...
    gl.enableVertexAttribArray(this.vTexCoord);
     ...
     //s is uniform?
    this.s = gl.getUniformLocation(this.shaderProgram, "s");
    this.scaler = gl.getUniformLocation(this.shaderProgram, "scaler");
  }
...
JSONModel.prototype.Redraw = function(matrixStack, projectionMatrix, lightPositions,
  ...
  if (this.hasTexCoords) {
    ...
    gl.uniform2fv(this.s, this.vertexTexMove);
    this.vertexScale = document.getElementById(this.scaleSliderID);
    gl.uniform2fv(this.scaler, vec2(this.vertexScale.value,this.vertexScale.value));
    this.vertexTexMove[0]+= 0.015;
    this.vertexTexMove[1]+=0.015;
  }
  gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexShininessBuffer);
...
JSONModel.prototype.Mipmap = function() {
    if(!this.clicked){
